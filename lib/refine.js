...
// PATCH mode — instead of rewriting the whole document (a large, throttle-prone
// output), the model emits ONE small <style> override block. Most taste fixes
// (type scale, CTA, spacing, hierarchy, color, nav legibility) are CSS, so this
// applies them via higher-specificity selectors appended to the page — a ~1-2KB
// output that completes even under heavy API load. Can't add markup/copy.
const patchPrompt = (html, issues, signatureMoment, instruction) => `You are improving a web page WITHOUT rewriting it. Output ONLY one minified <style> block that, appended to the page, applies the fixes via MORE-SPECIFIC CSS selectors that override the existing rules.

Rules:
- Pure CSS only — no markup, no JS. Target the real elements / classes / ids visible in the HTML below.
- Reuse the page's existing var(--*) custom properties where present; add new --vars on :root only if needed.
- Honor prefers-reduced-motion for any motion. Be surgical — only what the fixes require.
${signatureMoment ? `- Build this signature moment in CSS if you can: ${signatureMoment}\n` : ''}${instruction ? `- Also apply: ${instruction}\n` : ''}
CRITIQUE FIXES:
${issues.length ? issues.map((i, n) => `${n + 1}. [${i.severity}] ${i.area} — ${i.observe} → ${i.fix}`).join('\n') : '(apply the instruction above)'}

HTML (read it for selectors + tokens; do NOT reproduce it):
${html}

Output ONLY the <style>…</style> block, then a line containing exactly ${SENTINEL}, then 2-5 short bullets of what you changed.`;

function applyPatch(html, styleText) {
  const m = String(styleText || '').match(/<style[\s\S]*?<\/style>/i);
  let block = m ? m[0] : `<style>${String(styleText || '').trim()}</style>`;
  block = block.replace(/<style/i, '<style data-db-refine="patch"');
  if (/<\/body>/i.test(html)) return html.replace(/<\/body>/i, block + '\n</body>');
  if (/<\/html>/i.test(html)) return html.replace(/<\/html>/i, block + '\n</html>');
  return html + block;
}

// Iteration-loop router: use the small-output PATCH unless the edit clearly needs
// NEW markup (add a section/footer/…) AND the page is small enough for a full
// rewrite to complete. Large pages always patch → throttle-safe; structural edits
// on a small page get the full rewrite that can actually add the markup.
export function shouldPatch(html, instruction) {
  const structural = /\b(add|insert|append|remove|delete|new section|section|footer|header|testimonials?|faq|pricing|gallery|carousel|another|extra|column)\b/i.test(String(instruction || ''));
  return !(structural && String(html || '').length < 9000);
}
 ...
 */
export async function refine({ html, instruction, critique: crit, verify = false, patch = false, vaultRoot, bookDir, shotsDir, model }) {
  ...
  try {
    const stream = sdk.query({
      prompt: (patch ? patchPrompt : refinePrompt)(html, issues, before ? before.signatureMoment : '', instruction),
  ...
  if (patch) {
    ...
    newHtml = applyPatch(html, i >= 0 ? text.slice(0, i) : text);
    changes = (i >= 0 ? text.slice(i + SENTINEL.length) : '')
      .split('\n').map((l) => l.replace(/^\s*[-*•]\s?/, '').trim()).filter(Boolean).slice(0, 8);
  } else {
    ({ html: newHtml, changes } = splitOutput(text));
  }
  if (!newHtml || newHtml.length < 200) return { error: 'refine produced no usable HTML', raw: text.slice(0, 400) };
  const truncated = !patch && !/<\/html\s*>/i.test(newHtml);   // missing closing tag ⇒ output cut off (full-rewrite only)
