...
export async function refine({ html, instruction, critique: crit, verify = false, vaultRoot, bookDir, shotsDir, model }) {
  ...
  // 1) targets — use provided critique, else critique now
  ...
  if (!issues.length && !instruction) {
    return { ok: true, html, changes: [], unchanged: true, note: 'no issues flagged and no instruction — nothing to refine', before: before.scores ? { scores: before.scores } : null };
  }
  ...
  try {
    const stream = sdk.query({
      prompt: refinePrompt(html, issues, before.signatureMoment, instruction),
  ...
  return {
    ...
    changes,
    truncated,
    before: before.scores ? { total: before.scores.total, scores: before.scores, verdict: before.verdict } : null,
    after,
    delta: (after && before.scores) ? after.total - before.scores.total : null,
    model: useModel,
  };
}
