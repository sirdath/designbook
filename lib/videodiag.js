...
// ---- contact sheet: tile labeled frame stills into ONE grid image, so an agent can
// be handed "the frames at a place" in a single picture (visual diagnosis). Uses
// ImageMagick montage (clean grid + per-tile captions; this ffmpeg has no drawtext)
// and falls back to an unlabeled ffmpeg tile if montage is absent. ----
const _CS_FONT = ['/System/Library/Fonts/Supplemental/Arial.ttf', '/System/Library/Fonts/Helvetica.ttc'].find((f) => existsSync(f)) || null;

function _ffmpegTile(paths, outPath, cols, width) {
  const tmp = mkdtempSync(join(tmpdir(), 'dbcs-'));
  const tiles = [];
  paths.forEach((fp) => {
    const t = join(tmp, `cs_${String(tiles.length).padStart(3, '0')}.png`);
    try { execFileSync('ffmpeg', ['-y', '-i', fp, '-vf', `scale=${width}:-2`, '-frames:v', '1', t], { stdio: 'ignore' }); tiles.push(t); } catch (e) { /* skip */ }
  });
  if (!tiles.length) { try { rmSync(tmp, { recursive: true, force: true }); } catch {} return { error: 'no frames to tile' }; }
  const rows = Math.ceil(tiles.length / cols);
  let ok = false;
  try { execFileSync('ffmpeg', ['-y', '-framerate', '1', '-i', join(tmp, 'cs_%03d.png'), '-vf', `tile=${cols}x${rows}:padding=10:margin=12:color=#0b0b0f`, '-frames:v', '1', outPath], { stdio: 'ignore' }); ok = existsSync(outPath); } catch (e) { /* below */ }
  try { rmSync(tmp, { recursive: true, force: true }); } catch {}
  return ok ? { path: outPath, cols, rows, count: tiles.length, labeled: false } : { error: 'tile failed' };
}

export function contactSheet(framePaths, outPath, opts = {}) {
  const width = opts.width || 420;
  const labels = opts.labels || framePaths.map((_, i) => String(i));
  const cols = Math.max(1, Math.min(opts.cols || 4, framePaths.length));
  const valid = framePaths.map((p, i) => ({ p, l: labels[i] })).filter((x) => existsSync(x.p));
  if (!valid.length) return { error: 'no frames' };
  const rows = Math.ceil(valid.length / cols);
  const args = [];
  for (const { p, l } of valid) args.push('-label', String(l || '').slice(0, 28), p); // -label applies to the NEXT image
  args.push('-font', _CS_FONT || 'Helvetica', '-pointsize', '24', '-fill', '#e8e8ec',
    '-tile', `${cols}x${rows}`, '-geometry', `${width}x+10+12`, '-background', '#0b0b0f', outPath);
  try { execFileSync('montage', args, { stdio: 'ignore' }); }
  catch (e) { return _ffmpegTile(valid.map((v) => v.p), outPath, cols, width); }
  return existsSync(outPath) ? { path: outPath, cols, rows, count: valid.length, labeled: !!_CS_FONT } : _ffmpegTile(valid.map((v) => v.p), outPath, cols, width);
}
