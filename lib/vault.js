...
export async function loadVault() {
  ...
  return {
    ...
    coherence(html) {
      ...
      const as = antiSlop(html);
      const extra = (ca.penalty || 0) + (as.penalty || 0);
      const warn = [...ca.findings.map((f) => f.message), ...as.findings.map((f) => f.message)];
      if (!extra && !warn.length) return { ...c, color: ca, antiSlop: as };
      const score = Math.max(0, Math.round((c.score || 0) - extra));
      return {
        ...c,
        score,
        ok: score >= 80,
