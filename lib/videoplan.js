...
export function composeVideoPlan(opts = {}) {
  ...
  const seq = GENRE_SEQUENCES[genre] || FALLBACK_SEQUENCE;
...
/** Total frames = sum of scene durations (transitions overlap, handled in Root). */
export function totalDuration(plan) {
  return (plan.scenes || []).reduce((n, s) => n + (s.durationInFrames || 0), 0);
}
