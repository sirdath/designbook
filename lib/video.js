...
import { videoCoherence, totalDuration } from './videoplan.js';
...
function sceneKeyframes(plan) {
  ...
  for (const s of plan.scenes || []) {
    out.push({ sceneId: s.id, type: s.type, frame: Math.min(totalDuration(plan) - 1, off + Math.floor((s.durationInFrames || 1) / 2)) });
    off += s.durationInFrames || 0;
  }
  return out;
}
