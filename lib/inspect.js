...
(function () {
  ...
  function run() {
    ...
    R.summary = { errors: errs, contrastFails: R.a11y.contrastFails.length, layoutIssues: (R.layout.hOverflow ? 1 : 0) + R.layout.overflowers.length + R.layout.overlaps.length, score: R.score };
...
export const MODES = ['layout', 'perf', 'diagnose', 'element'];
...
export async function inspect({ html, vaultRoot, bookDir, viewports, screenshot, fullPage, shotsDir, label, mode, selector }) {
  ...
  const perf = m === 'perf';
  const single = perf || m === 'element';
  if (m === 'element' && !selector) return { error: 'element mode requires a selector' };
  const names = single
    ? [(viewports && viewports[0]) || 'desktop'].filter((n) => VIEWPORTS[n])
  ...
  let probe = { layout: PROBE, perf: PERF_PROBE, diagnose: DIAGNOSE_PROBE, element: ELEMENT_PROBE }[m];
  ...
  try {
    if (perf) {
      // perf samples the real frame clock — direct full-window render
      ...
      try {
        ...
        const m = stdout.match(/__DBPROBE__([\s\S]*?)__END__/);
        report = m ? JSON.parse(m[1]) : { error: 'probe produced no output' };
      } catch (e) {
        report = { error: 'render failed: ' + (e.message || e) };
      }
      reports.push({ viewport: names[0], ...VIEWPORTS[names[0]], mode: 'perf', ...report });
    } else {
