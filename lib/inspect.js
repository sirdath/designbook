...
// Bounded Chrome: a FIFO semaphore so concurrent inspects/gates/briefs can't
// fork an unbounded number of headless Chrome processes (each ~150MB). Excess
// spawns queue and run as slots free. 2 keeps the lab responsive without
// starving the owner's foreground work.
const CHROME_CONCURRENCY = 2;
let chromeActive = 0;
const chromeQueue = [];
function acquireChrome() {
  if (chromeActive < CHROME_CONCURRENCY) { chromeActive++; return Promise.resolve(); }
  return new Promise((resolve) => chromeQueue.push(resolve));
}
function releaseChrome() {
  chromeActive = Math.max(0, chromeActive - 1);
  const next = chromeQueue.shift();
  if (next) { chromeActive++; next(); }
}
async function runChrome(chrome, args, opts) {
  await acquireChrome();
  try { return await execFileP(chrome, args, opts); }
  finally { releaseChrome(); }
}

// Render cache — fact renders are content-addressed (the doc hash already encodes
// html+mode+selector), so the same inspect within a short window can reuse its
// reports instead of re-spawning Chrome. The draft→inspect→save loop renders the
// SAME html for the gate's diagnose and the agent's inspect; this collapses that
// to one render. Only the !screenshot path is cached (screenshots have a PNG
// side-effect). 60s TTL.
const RENDER_CACHE_MS = 60_000;
const renderCache = new Map();
function cacheGet(key) {
  const e = renderCache.get(key);
  if (!e) return null;
  if (Date.now() > e.expires) { renderCache.delete(key); return null; }
  // fresh top-level wrapper per hit — the caller (server.js) stamps fields like
  // autoRoutedMobile onto the result; mutating the cached object would leak that
  // across requests. reports[] are read-only on the cached (!screenshot) path.
  return { ...e.value };
}
function cacheSet(key, value) {
  renderCache.set(key, { value, expires: Date.now() + RENDER_CACHE_MS });
  if (renderCache.size > 64) { const k = renderCache.keys().next().value; renderCache.delete(k); }
}
...
(function () {
  ...
  // ---- effective background, INCLUDING single linear-gradients ----
  // bgOf used to give up the moment text sat on any backgroundImage — i.e. blind
  // on exactly the mesh/aurora/hero-scrim surfaces the vault composes by default.
  // For a single linear-gradient we now evaluate the ACTUAL colour where the text
  // sits (project the glyph centre onto the gradient line + interpolate), so the
  // WCAG check becomes true there instead of skipping. Radial/conic/image/layered
  // are still unprovable → null (skip, as before).
  function lerp(a, b, t) { return a + (b - a) * t; }
  function dirToAngle(d) {
    var map = { 'top': 0, 'right': 90, 'bottom': 180, 'left': 270,
      'top right': 45, 'right top': 45, 'bottom right': 135, 'right bottom': 135,
      'bottom left': 225, 'left bottom': 225, 'top left': 315, 'left top': 315 };
    var v = map[d.trim()]; return v === undefined ? 180 : v;
  }
  function firstLinear(bi) {
    var i = bi.indexOf('linear-gradient(');
    if (i !== 0) return null;                       // must be the top (and only) layer
    var depth = 0, j = i + 'linear-gradient'.length;
    for (; j < bi.length; j++) { if (bi[j] === '(') depth++; else if (bi[j] === ')') { depth--; if (depth === 0) { j++; break; } } }
    if (bi.slice(j).trim().charAt(0) === ',') return null;   // layered → can't prove
    return bi.slice(i, j);
  }
  function gradStops(str) {
    var re = /(rgba?\\([^)]*\\))\\s*([\\d.]+%)?/g, m, raw = [];
    while ((m = re.exec(str))) { var c = parseRgb(m[1]); if (c) raw.push({ c: c, pos: m[2] ? parseFloat(m[2]) / 100 : null }); }
    if (raw.length < 2) return null;
    if (raw[0].pos === null) raw[0].pos = 0;
    if (raw[raw.length - 1].pos === null) raw[raw.length - 1].pos = 1;
    for (var i = 1; i < raw.length - 1; i++) if (raw[i].pos === null) {
      var p = i - 1; while (raw[p].pos === null) p--;
      var n = i + 1; while (n < raw.length && raw[n].pos === null) n++;
      raw[i].pos = raw[p].pos + (raw[n].pos - raw[p].pos) * (i - p) / (n - p);
    }
    return raw;
  }
  function colorAt(stops, p) {
    if (p <= stops[0].pos) return stops[0].c;
    if (p >= stops[stops.length - 1].pos) return stops[stops.length - 1].c;
    for (var i = 1; i < stops.length; i++) if (p <= stops[i].pos) {
      var a = stops[i - 1], b = stops[i], t = (p - a.pos) / ((b.pos - a.pos) || 1);
      return { rgb: [lerp(a.c.rgb[0], b.c.rgb[0], t), lerp(a.c.rgb[1], b.c.rgb[1], t), lerp(a.c.rgb[2], b.c.rgb[2], t)], a: lerp(a.c.a, b.c.a, t) };
    }
    return stops[stops.length - 1].c;
  }
  function evalLinear(grad, box, px, py) {
    var lp = grad.indexOf('('), rp = grad.lastIndexOf(')');
    if (lp < 0 || rp < 0) return null;
    var inner = grad.slice(lp + 1, rp), angle = 180, stopsStr = inner;
    var am = inner.match(/^\\s*(-?[\\d.]+)deg\\s*,/), dm = inner.match(/^\\s*to\\s+([a-z]+(?:\\s+[a-z]+)?)\\s*,/);
    if (am) { angle = parseFloat(am[1]); stopsStr = inner.slice(am[0].length); }
    else if (dm) { angle = dirToAngle(dm[1]); stopsStr = inner.slice(dm[0].length); }
    var stops = gradStops(stopsStr); if (!stops) return null;
    var a = angle * Math.PI / 180, dx = Math.sin(a), dy = -Math.cos(a);
    var lineLen = Math.abs(box.w * Math.sin(a)) + Math.abs(box.h * Math.cos(a));
    if (!lineLen) return null;
    var rx = px - box.x - box.w / 2, ry = py - box.y - box.h / 2;
    var pos = (rx * dx + ry * dy) / lineLen + 0.5;
    return colorAt(stops, Math.max(0, Math.min(1, pos)));
  }
  function composite(over, under) {
    if (!over) return under;
    if (over.a >= 0.999) return over.rgb;
    var t = over.a;
    return [over.rgb[0] * t + under[0] * (1 - t), over.rgb[1] * t + under[1] * (1 - t), over.rgb[2] * t + under[2] * (1 - t)];
  }
  function solidBelow(node) {
    var n = node;
  ...
  function bgOf(el, px, py) {
    ...
    while (node && node !== document.documentElement) {
      ...
      if (bi && bi !== 'none') {
        var grad = (px !== undefined) ? firstLinear(bi.trim()) : null;
        if (grad) {
          var b = node.getBoundingClientRect();
          var col = evalLinear(grad, { x: b.left, y: b.top, w: b.width, h: b.height }, px, py);
          if (col) return composite(col, solidBelow(node.parentElement));
        }
        return null;                                 // radial/conic/image/layered → unprovable
      }
  ...
  function run() {
    ...
    var R = {
      ...
      a11y: { contrastFails: [], missingAlt: [], unlabeledInputs: [], namelessButtons: [], headingSkips: [], duplicateIds: [], invalidRoles: [], badAriaAttrs: [], positiveTabindex: [] },
      layout: { hOverflow: (document.scrollingElement || document.documentElement).scrollWidth > W + 1, overflowers: [], smallTaps: [], tinyText: [], overlaps: [], occluded: [], asymmetry: [], balanced: true },
      typography: { longLines: [], fontFamilies: [] },
      css: { undefinedVars: [], unloadedFonts: [] },
      images: { oversized: [], missingDims: [], belowFoldEager: [], lcpNotPreloaded: null },
    ...
    for (var i = 0; i < all.length; i++) {
      ...
      var tag = el.tagName;
      // ---- ARIA validity + focus order: the biggest a11y bug class a pixel-blind
      // composer emits (invalid role, typo'd aria-* that silently does nothing,
      // tabindex>0 that scrambles tab order). Cheap + deterministic, no axe needed.
      var roleAttr = el.getAttribute && el.getAttribute('role');
      if (roleAttr && R.a11y.invalidRoles.length < 8) {
        var role0 = roleAttr.trim().split(/\\s+/)[0];
        if (role0 && VALID_ROLES.indexOf(role0) === -1) R.a11y.invalidRoles.push({ sel: sel(el), role: role0 });
      }
      if (el.hasAttribute && el.hasAttribute('tabindex') && R.a11y.positiveTabindex.length < 8) {
        var ti = parseInt(el.getAttribute('tabindex'), 10);
        if (ti > 0) R.a11y.positiveTabindex.push({ sel: sel(el), tabindex: ti });
      }
      if (el.attributes && R.a11y.badAriaAttrs.length < 8) {
        for (var aai = 0; aai < el.attributes.length; aai++) {
          var anm = el.attributes[aai].name;
          if (anm.indexOf('aria-') === 0 && VALID_ARIA.indexOf(anm) === -1) { R.a11y.badAriaAttrs.push({ sel: sel(el), attr: anm }); break; }
        }
      }
      ...
      if (tag === 'IMG') {
        ...
        // perf: below-the-fold images should lazy-load (don't block the LCP)
        if (vis && r.top >= window.innerHeight && el.getAttribute('loading') !== 'lazy' && R.images.belowFoldEager.length < 8) R.images.belowFoldEager.push(sel(el));
      }
      ...
      if (vis && textChecked < 400 && el.childElementCount === 0 && el.textContent && el.textContent.trim().length > 2) {
        ...
        var bg = fg && fg.a > 0.4 ? bgOf(el, r.left + r.width / 2, r.top + r.height / 2) : null;
        if (fg && bg) {
    ...
    // default-font tell: Inter/Roboto as the ONLY custom (non-system) face — the
    // typographic third of the AI-default trio. Pure-system (minimal) is exempt;
    // any real display+text pairing clears it.
    var sysFont = /^(system-ui|-apple-system|blinkmacsystemfont|segoe ui|sans-serif|serif|monospace|ui-monospace|ui-sans-serif|ui-serif|inherit|initial|)$/i;
    var customFaces = R.typography.fontFamilies.filter(function (f) { return f && !sysFont.test(f); });
    if (customFaces.length === 1 && /^(inter|roboto)$/i.test(customFaces[0])) R.typography.defaultFont = customFaces[0];
    // perf (CWV): the largest above-the-fold image is the LCP candidate — it should
    // be prioritized (fetchpriority=high or a preload link), or LCP drags.
    try {
      var lcp = null;
      Array.prototype.forEach.call(document.images, function (im) {
        var ir = im.getBoundingClientRect();
        if (ir.width < 2 || ir.height < 2 || ir.top >= window.innerHeight || ir.bottom <= 0) return;
        var a = ir.width * ir.height;
        if (a > 40000 && (!lcp || a > lcp.a)) lcp = { el: im, a: a };
      });
      if (lcp) {
        var hiP = lcp.el.getAttribute('fetchpriority') === 'high';
        var bn = (lcp.el.getAttribute('src') || '').split('/').pop().split('?')[0];
        var pre = bn && Array.prototype.some.call(document.querySelectorAll('link[rel="preload"]'), function (l) { return (l.getAttribute('href') || '').indexOf(bn) !== -1; });
        if (!hiP && !pre) R.images.lcpNotPreloaded = { sel: sel(lcp.el) };
      }
    } catch (e) {}
    ...
    // ---- occlusion: a primary control whose CENTER is covered by an UNRELATED
    // element (decorative hero / leftover scrim / ::after band over the CTA). A
    // human can't click it, yet every other fact reads "fine". Above-the-fold only
    // (elementFromPoint is viewport-bound; the static render doesn't scroll). ----
    for (var oi = 0; oi < interactive.length && R.layout.occluded.length < 6; oi++) {
      var it = interactive[oi], ir = it.r;
      if (ir.top >= window.innerHeight || ir.bottom <= 0 || ir.width < 10 || ir.height < 10) continue;
      var hasPurpose = (it.el.textContent && it.el.textContent.trim().length > 0) || it.el.getAttribute('aria-label') || it.el.querySelector('img,svg');
      if (!hasPurpose) continue;
      var cx = Math.max(0, Math.min(window.innerWidth - 1, ir.left + ir.width / 2));
      var cy = Math.max(0, Math.min(window.innerHeight - 1, ir.top + ir.height / 2));
      var hit = document.elementFromPoint(cx, cy);
      if (!hit || hit.id === '__dbprobe') continue;
      // clickable when the hit IS the control, sits inside it, or is a wrapper that
      // contains it (a full-card <a> around a button is intentional, not occlusion).
      if (hit === it.el || it.el.contains(hit) || hit.contains(it.el)) continue;
      R.layout.occluded.push({ sel: sel(it.el), by: sel(hit), sample: (it.el.textContent || '').trim().slice(0, 24) });
    }
    ...
    // ---- symmetry: unintended horizontal imbalance (a debugging aid). Asymmetry is
    // often deliberate (editorial / split layouts), so we flag ONLY high-signal cases:
    // wide blocks with mismatched left/right padding (a typo), blocks the layout
    // DECLARES centered (inline margin:auto / flex justify-center) yet that render
    // off-center, and a page-level skew read. Low false-positive by design. ----
    try {
      function padOf(cs, side) { return parseFloat(cs['padding' + side]) || 0; }
      var symCands = document.querySelectorAll('section, header, footer, main, article, [class*="container"], [class*="wrapper"], [class*="inner"], [class*="content"], [class*="row"]');
      var symSeen = 0;
      for (var syi = 0; syi < symCands.length && R.layout.asymmetry.length < 6 && symSeen < 240; syi++) {
        var se = symCands[syi];
        if (se.id === '__dbprobe') continue;
        var sr = se.getBoundingClientRect();
        if (sr.width < W * 0.5 || sr.height < 36 || sr.bottom < 0 || sr.top > document.documentElement.scrollHeight) continue;
        var scs = getComputedStyle(se);
        if (scs.position === 'absolute' || scs.position === 'fixed' || scs.float !== 'none' || scs.display === 'none') continue;
        symSeen++;
        // A) padding asymmetry — both sides padded ⇒ symmetric intent; a mismatch is a typo
        var padL = padOf(scs, 'Left'), padR = padOf(scs, 'Right');
        if (padL > 4 && padR > 4) {
          var pdiff = Math.abs(padL - padR);
          if (pdiff > 10 && pdiff > 0.25 * Math.min(padL, padR)) {
            R.layout.asymmetry.push({ sel: sel(se), kind: 'uneven-padding', left: Math.round(padL), right: Math.round(padR),
              note: 'horizontal padding ' + Math.round(padL) + 'px left vs ' + Math.round(padR) + 'px right — sections are usually symmetric; likely a typo.' });
            continue;
          }
        }
        // B) declared centered (margin:auto inline, or flex justify-center parent) but off-center
        var par = se.parentElement;
        if (par) {
          var pcs = getComputedStyle(par), pr = par.getBoundingClientRect();
          var marginAuto = se.style.marginLeft === 'auto' && se.style.marginRight === 'auto';
          var flexCenter = pcs.display.indexOf('flex') >= 0 && pcs.justifyContent === 'center' && pcs.flexDirection.indexOf('row') >= 0;
          if ((marginAuto || flexCenter) && sr.width < pr.width - 16) {
            var pcl = pr.left + padOf(pcs, 'Left'), pcr = pr.right - padOf(pcs, 'Right');
            var lg = sr.left - pcl, rg = pcr - sr.right, gap = Math.abs(lg - rg);
            if (gap > 14 && gap > 0.02 * W) {
              R.layout.asymmetry.push({ sel: sel(se), kind: 'off-center', leftGap: Math.round(lg), rightGap: Math.round(rg),
                note: 'declared centered (' + (marginAuto ? 'margin:auto' : 'flex justify-center') + ') but sits ' + Math.round(lg) + 'px from left vs ' + Math.round(rg) + 'px from right — something is pushing it aside.' });
            }
          }
        }
      }
      // C) page-level balance — is the primary content centered in the viewport?
      var bodyKids = Array.prototype.filter.call(document.body.children, function (c) {
        if (c.id === '__dbprobe') return false;
        var cs = getComputedStyle(c); if (cs.position === 'fixed' || cs.position === 'absolute' || cs.display === 'none') return false;
        var b = c.getBoundingClientRect(); return b.width > W * 0.3 && b.height > 60;
      });
      if (bodyKids.length && R.layout.asymmetry.length < 6) {
        var minL = 1e9, maxR = -1e9;
        bodyKids.forEach(function (c) { var b = c.getBoundingClientRect(); if (b.left < minL) minL = b.left; if (b.right > maxR) maxR = b.right; });
        var lGap = minL, rGap = W - maxR;
        if (lGap > 2 && rGap > 2) {
          var g = Math.abs(lGap - rGap);
          if (g > 24 && g > 0.04 * W) R.layout.asymmetry.push({ sel: 'body', kind: 'page-skew', leftGap: Math.round(lGap), rightGap: Math.round(rGap),
            note: 'page content is skewed: ' + Math.round(lGap) + 'px gap on the left vs ' + Math.round(rGap) + 'px on the right — not centered in the viewport.' });
        }
      }
      R.layout.balanced = R.layout.asymmetry.length === 0;
    } catch (e) {}
    ...
    penalty += R.a11y.invalidRoles.length * 4 + R.a11y.badAriaAttrs.length * 3 + R.a11y.positiveTabindex.length * 2;
    penalty += (R.layout.hOverflow ? 10 : 0) + R.layout.overflowers.length * 3 + R.layout.overlaps.length * 4 + R.layout.tinyText.length * 2 + R.layout.occluded.length * 8 + R.layout.asymmetry.length * 4;
    penalty += R.css.undefinedVars.length * 4 + R.css.unloadedFonts.length * 3 + (R.typography.defaultFont ? 6 : 0);
    penalty += R.images.oversized.length * 2 + R.images.missingDims.length * 2 + R.images.belowFoldEager.length + (R.images.lcpNotPreloaded ? 4 : 0);
    ...
    R.summary = { errors: errs, contrastFails: R.a11y.contrastFails.length, invisibleContent: R.render.invisibleContent.length, layoutIssues: (R.layout.hOverflow ? 1 : 0) + R.layout.overflowers.length + R.layout.overlaps.length + R.layout.occluded.length + R.layout.asymmetry.length, asymmetry: R.layout.asymmetry.length, score: R.score };
...
// Runs under Chrome's --force-prefers-reduced-motion: any INFINITE css animation
// still live here has no reduced-motion guard (the vault's policy is that every
// animated snippet honors reduce; nothing verified composed pages until now).
const REDUCE_PROBE = `
<pre id="__dbprobe" style="display:none"></pre>
<script id="__dbreduce">
(function(){
  function selOf(el){
    if (el.id) return '#' + el.id;
    var cls = el.className; if (cls && cls.baseVal !== undefined) cls = cls.baseVal;
    var c = String(cls || '').trim().split(/\\s+/)[0];
  ...
  function go(){
    ...
    for (var i = 0; i < all.length && out.length < 12; i++){
      ...
      if (r.top > window.innerHeight * 3 || r.bottom < -window.innerHeight) continue;
      var cs = getComputedStyle(el);
      var name = cs.animationName;
      if (!name || name === 'none') continue;
      var inf = cs.animationIterationCount.split(',').some(function(v){ return v.trim() === 'infinite'; });
      var dur = 0; cs.animationDuration.split(',').forEach(function(v){ var n = parseFloat(v) || 0; if (n > dur) dur = n; });
      if (inf && dur > 0.05){
        var s = selOf(el);
        if (!seen[s]){ seen[s] = 1; out.push({ sel: s, animation: name.split(',')[0].trim(), duration: cs.animationDuration.split(',')[0].trim() }); }
      }
    }
    var pre = document.getElementById('__dbprobe');
    if (pre) pre.textContent = '__DBPROBE__' + JSON.stringify({ emulatedReduce: reduce, infiniteUnderReduce: out }) + '__END__';
  }
  if (document.readyState !== 'loading') setTimeout(go, 30);
  else addEventListener('DOMContentLoaded', function(){ setTimeout(go, 30); });
})();
</script>`;
...
(function () {
  ...
  var TABBAR_SHELLS = { feed: 1, dashboard: 1, profile: 1, list: 1, settings: 1, fablist: 1, empty: 1 };
  var FULLSCREEN_SHELLS = { onboarding: 1, auth: 1, paywall: 1, checkout: 1, splash: 1, player: 1, sheet: 1 };
...
(function () {
  ...
  function run() {
    ...
    // headings define the display scale even when they wrap child elements (e.g.
    // a per-letter wordmark, where each <span> is too short to be a text leaf) —
    // measure them directly so big display type is never missed.
    var heads = document.querySelectorAll('h1, h2, h3');
    for (var hi = 0; hi < heads.length; hi++) {
      var hr = heads[hi].getBoundingClientRect();
      if (!(hr.width > 0 && hr.height > 0)) continue;
      var hfs = Math.round(num(getComputedStyle(heads[hi]).fontSize));
      if (hfs) { sizeSet[hfs] = (sizeSet[hfs] || 0) + 1; headingSizes.push(hfs); }
    }
...
export async function inspect({ html, vaultRoot, bookDir, viewports, screenshot, fullPage, shotsDir, label, mode, selector }) {
  ...
  const id = hashOf(doc);
  // content-addressed cache: same doc + same viewports within 60s → reuse facts
  // (the gate's diagnose render and the agent's inspect hit this). Never for
  // screenshots — those must actually write the PNG.
  const cacheKey = `${id}:${names.join(',')}`;
  if (!screenshot) { const hit = cacheGet(cacheKey); if (hit) return hit; }
...
(function () {
  ...
  var iv = setInterval(function () {
    ...
    // ---- reduced-motion HONORING pass (diagnose only) ----
    // One extra render with the OS reduce flag forced. Any infinite CSS animation
    // still live then has no prefers-reduced-motion guard — a real a11y defect the
    // gate was blind to (string-presence in audit.js ≠ semantic verification).
    // Best-effort: never blocks the main report; folded into the cached result.
    if (m === 'diagnose' && reports.length) {
      try {
        let rdoc = String(html || '');
        if (bookDir) {
          const bu = 'file://' + encodeURI(bookDir).replace(/#/g, '%23');
          rdoc = rdoc.split('"/book/').join('"' + bu + '/').split("'/book/").join("'" + bu + '/');
        }
        rdoc = injectBase(rdoc, fileBase);
        rdoc = rdoc.includes('</body>') ? rdoc.replace('</body>', REDUCE_PROBE + '\n</body>') : rdoc + REDUCE_PROBE;
        const rtmp = join(tmpdir(), `dbinspect-${id}-reduce.html`);
        writeFileSync(rtmp, rdoc);
        cleanup.push(rtmp);
        const { stdout } = await runChrome(chrome, [
          '--headless=new', '--disable-gpu', '--no-sandbox', '--hide-scrollbars',
          '--force-prefers-reduced-motion',
          `--window-size=${Math.max(VIEWPORTS[names[0]].width, 500)},1200`,
          '--virtual-time-budget=4000', '--dump-dom', 'file://' + rtmp,
        ], { maxBuffer: 32 * 1024 * 1024, timeout: 20000 });
        const rm = stdout.match(/__DBPROBE__([\s\S]*?)__END__/);
        if (rm) { const parsed = JSON.parse(rm[1]); for (const rep of reports) rep.reducedMotion = parsed; }
      } catch { /* best-effort; reduced-motion is a warning dimension, never blocks */ }
    }
